...
class Trainer(object):
    ...
    def rebuild_loss(self, use_gumbel=False):
        x = self.model.node_embeddings
        adj = self.model.adj
        L_tilde_learned = self.model.tilde
        h, w = L_tilde_learned.shape
        if use_gumbel:
            loss = F.smooth_l1_loss(self.adj_mx, adj)
        else:
            L_tilde_learned = F.softmax(F.relu(torch.mm(x, x.transpose(0, 1))), dim=1)
            loss = F.smooth_l1_loss(self.L_tilde, L_tilde_learned)
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        np.save('./{}_true.npy'.format(args.dataset), y_true.cpu().numpy())#真实值
        np.save('./{}_pred.npy'.format(args.dataset), y_pred.cpu().numpy())#预测值
